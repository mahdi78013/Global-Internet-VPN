...
def print_banner(version: str, *, stream=None) -> None:
    ...
    version_line = f"[ اینترنت آزاد • Free Internet • v{version} ]"
    subtext = f"by اینترنت آزاد 369"
    link = "https://t.me/muntivpn"

    # Calculate width from banner lines
    width = max(max(len(line) for line in art), len(version_line), len(subtext), len(link))
    rule = "─" * width
    ...
    if color:
        ...
        for line in art:
            if line:
                print(f"{BOLD}{FG_GREEN}{line.center(width)}{RESET}", file=stream)
            else:
                print("", file=stream)
        print(f"{FG_GREEN}{version_line.center(width)}{RESET}", file=stream)
        print(f"{DIM}{FG_GREEN}{subtext.center(width)}{RESET}", file=stream)
        ...
        print(subtext.center(width), file=stream)
